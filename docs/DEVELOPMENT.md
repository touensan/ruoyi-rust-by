...
- 本仓库仅文档修正，未改变运行代码或依赖；按文档范围核对链接、版本措辞及历史附件，未重复运行与此改动无关的功能构建。

## 2026-09-24｜单机平滑更新源码

- 新增 `deploy/smooth-release.py`、配置模板与 `SMOOTH_RELEASE.md`；候选 `/internal/ready` 检查 MySQL 和已应用迁移数，服务监听由 `RUOYI_PORT` 限定回环。SIGTERM 优雅关闭 HTTP，调度循环在当前任务完成后退出；现有数据库行锁仍阻止双实例同一任务重复执行。
- 打包脚本为每个内部文件生成摘要。工具核对归档、内部文件、架构和旧静态资源，使用 Nginx reload 切流并保留旧进程。旧 v0.2.0 下载包未改；没有部署任何业务站点。
- 本机内存余量较小，按资源探测在深圳隔离目录与项目专用缓存中执行。Rust 1.98.1 `cargo check --locked`、`cargo test --locked`（14 项通过）及 `cargo fmt` 通过，依赖从 rsproxy.cn 镜像获取。Python 工具 3 项完整性/准备测试及隔离 Nginx 片段语法检查通过；同一工具在隔离双后端 Nginx 切流演练中旧慢请求正常完成、新请求抵达候选。未做真实 MySQL/Redis 双实例、验证码切流或正式访客高并发验收。
- 验证码与限速仍为实例内存，旧版取到的验证码跨切流可能失效；因此当前代码不能承诺所有登录无感。破坏性 schema 与不兼容 API 变更也不能直接平滑发布。

## 2026-09-24｜v0.2.1 Release

- v0.2.1 将上述单机平滑更新能力作为新的补丁版本发布；v0.2.0 与 v0.1.0 历史标签和附件保持不变。本次只发布 GitHub 运行包，不升级业务站。
- 后端从 v0.2.1 标签源码在深圳隔离构建节点使用 Rust 1.98.1 构建 Linux x86_64 musl 二进制；前端源码相对 v0.2.0 未变化，复用并核对 v0.2.0 已发布前端文件摘要。最终包包含逐文件摘要、构建来源、最新发布工具和文档。
