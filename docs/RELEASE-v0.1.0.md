...
这是预发布版本：支付网关、SMTP 发送、Redis/定时任务、Excel 导入和完整 CRUD 生成尚未实现。生产前请阅读 `docs/FEATURES.md`、`SECURITY.md` 和 `docs/VALIDATION.md`，其中明确记录未修复的上游 RSA 与前端构建依赖审计问题。生产前端依赖审计为零报告；CI 模板尚未激活。
