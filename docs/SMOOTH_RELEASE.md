...
v0.2.1 提供双端口发布流程；v0.2.0 及以前下载包不包含。应用升级期间新旧二进制并行，候选在回环备用端口通过 `/internal/ready` 的数据库与版本检查后，Nginx 平滑 reload。旧 worker 完成连接后旧进程才可退出。共享 `uploads` 与 `/admin/assets/` 中保留的历史 hash 文件避免上传或旧页面资源丢失。工具不会自动删旧制品。
...
python3 deploy/smooth-release.py --config /private/release.json prepare release-yyyymmdd /build/ruoyi-rust-by.tar.gz ARCHIVE_SHA256 0.2.1
