...
const initiallyCollapsed = props.defaultCollapsed || window.matchMedia("(max-width: 767px)").matches
const collapsed = ref<boolean>(initiallyCollapsed)
const sidebarWidth = ref<number>(initiallyCollapsed ? props.collapsedWidth : props.defaultWidth)
...
const onNodeClick = (data: any, node: any, e: Event): void => {
  ...
  if (window.matchMedia('(max-width: 767px)').matches) collapsed.value = true
}
