...
export function usePasswordRule() {
  ...
  const pwdValidator = computed(() => {
    ...
    return [
      ...
      { min: 12, max: 72, message: '密码长度必须介于 12 和 72 之间', trigger: 'blur' },
  ...
  const pwdPromptValidator = (value: string) => {
    ...
    if (!value || value.length < 12 || value.length > 72) {
      return '密码长度必须介于 12 和 72 之间'
    }
  ...
  const infoPwdValidator = computed(() => {
    ...
    return [
      ...
      { min: 12, max: 72, message: '新密码长度必须介于 12 和 72 之间', trigger: 'blur' },
      { pattern: rule.pattern, message: rule.message, trigger: 'blur' }
    ]
  })
  // 注册页面密码校验
  const registerPwdValidator = computed(() => {
    ...
    return [
      ...
      { min: 12, max: 72, message: '用户密码长度必须介于 12 和 72 之间', trigger: 'blur' },
      { pattern: rule.pattern, message: rule.message, trigger: 'blur' }
    ]
  })
